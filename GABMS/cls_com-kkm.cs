...
namespace GABMS
{
    class cls_com
    {
        public static string g시스템명 = "GABMS";
        public static string g상호 = "한국방송통신대학교";

        public static string sql;

        public static string gConn_String = "Data Source=49.1.232.110,5000;Initial Catalog=GABMS;User Id=syst5132;Password=tltmxm5132!@#";
        ...
        public static DataSet Select_Query2(string sQuery)
        {
            try
            {

                DataSet ds = new DataSet();
                SqlDataAdapter sqlAdapter = new SqlDataAdapter(sQuery, gConn);
                sqlAdapter.Fill(ds);
                return ds;
            }
            catch (SqlException ex)
        ...
        public static string ExcuteNonQuery2(string sQuery)
        {
            try
            {
                SqlCommand com = new SqlCommand(sQuery, gConn);
                com.ExecuteNonQuery();
                string sResult = "success";
                return sResult;
            }
        ...
        public static void 높이자동조절(FarPoint.Win.Spread.SheetView spr )
        {
            Application.DoEvents();
            for (int i = 0; i < spr.RowCount; i++)
            {
                spr.Rows[i].Height = spr.Rows[i].GetPreferredHeight();
            }
        }
